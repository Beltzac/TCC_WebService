...
%   [Z, R] = RESIZEM(Z1, [NUMROWS NUMCOLS], R1) resizes a regular data grid
%   with a referencing vector or matrix, R1, and returns a new grid and its
%   referencing vector or matrix, R. R1 is either a 1-by-3 vector
%   containing elements:
%
...
%   along a meridian and each row falls along a parallel. If R1 is a
%   referencing vector, then R is a referencing vector. Likewise, if R1 is
%   a referencing matrix, then R is a referencing matrix. If R1 is a
%   referencing vector, then SCALE must be a scalar resizing factor; the
%   form [NUMROWS NUMCOLS] is supported only for referencing matrices.
%
...
% $Revision: 1.9.4.8 $  $Date: 2009/05/14 17:05:30 $
...
% Construct a GeoRasterRefKernel object for the original input grid,
% then reset it to the new size.  The latitude and longitude limits,
% and the column and row directions, will be preserved.
S = convertToGeoRasterRef(R, originalSize, 'degrees', mfilename, 'R', 2);
S.RasterSize = newSize;

% Construct either a new referencing vector or a new referencing matrix.
if refvecInput
    % Referencing vector
...
if ndims(map0) > 2
    error(['map:' mfilename ':mapError'], ...
        'Input map can not contain pages')

elseif any(nonzeros(map0) ~= 1)
    error(['map:' mfilename ':mapError'], ...
        'Input map must be sparse and binary')
end
